...
public class AddressDaoH2 implements IDao<Address> {
    ...
    public void update(Address address) {
        ...
        try { //intenta hacer lo q intentamos
            connection = DB.getConnection();
            ...
            e.printStackTrace(); //si falla muestra
        } finally { //si todo esta ok entra aca
            try {
                connection.close(); //y cierra la conexion
            } catch (Exception e) {
                e.printStackTrace();//si falla muestra
            }
        }
