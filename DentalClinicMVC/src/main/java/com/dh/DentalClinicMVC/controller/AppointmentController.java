...
public class AppointmentController {
    ...
    @Autowired // no es necesario desde una nueva version de spring
    ...
    public ResponseEntity<Appointment> save(@RequestBody Appointment appointment) {
        ...
        if (dentistService.findById(appointment.getDentist().getId()).isPresent() && patientService.findById(appointment.getPatient().getId()).isPresent()) {
            //seteamos al response entity con codigo 200 y le agregamos el
            //turno como cuerpo de la respuesta
            response = ResponseEntity.ok(appointmentService.save(appointment));
        } else {
            //setear al response entity el codigo 400 y lo construimos
            // con un build
